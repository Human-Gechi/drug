...
def extract_count(query: str) -> int | None:
...
def resolve_limit(query: str, explicit: int | None, default: int) -> int:
...
def priority_terms(query: str) -> list[str]:
    return sorted(
        {t[:6] for t in content_tokens(query) if len(t) >= 4 and t not in GENERIC_TERMS}
    )
...
def collect_alerts(documents: list[PageDocument]) -> list[dict[str, Any]]:
    ...
    out.sort(
        key=lambda x: x["dt"] or datetime.min.replace(tzinfo=timezone.utc), reverse=True
    )
    return out


def build_chunks(
    documents: list[PageDocument], size: int = 900, overlap: int = 150
) -> list[dict[str, Any]]:
    ...
    for doc in documents:
        ...
        for i in range(0, max(len(text), 1), step):
            ...
            if piece.strip():
                chunks.append(
                    {"document": doc, "chunk": f"{doc.title}\n{piece}", "kind": "page"}
                )
...
def rank_documents(
    query: str, documents: list[PageDocument], top_k: int = 6
) -> list[dict[str, Any]]:
    ...
    if tokenized and q_tokens:
        ...
        ranked = sorted(
            zip(scores, [c for c, _ in tokenized]), key=lambda x: x[0], reverse=True
        )
